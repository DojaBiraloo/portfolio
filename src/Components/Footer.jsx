...
// ── Three.js scene — only mounted when footer is visible ─────────────
function BananaCanvas({ containerRef }) {
  useEffect(() => {
    const container = containerRef.current;
    if (!container) return;

    // Dynamically import Three.js + GLTFLoader only when needed
    let animId;
    let renderer;
    let cleanup;
...
export default function Footer() {
  ...
  // ── Observe footer — only load Three.js when it enters the viewport ─
  useEffect(() => {
  ...
  // ── GSAP scroll animations (no Three.js dependency) ─────────────────
  useEffect(() => {
    const footer = footerRef.current;
    if (!footer) return;
  ...
  return (
    <main className="overflow-x-hidden">
      ...
      >
        ...
        {/* THREE CANVAS — div always present for sizing; scene mounts only when visible */}
        <div ref={canvasRef} className="pointer-events-none absolute inset-0 z-0">
          {canvasVisible && <BananaCanvas containerRef={canvasRef} />}
        </div>
